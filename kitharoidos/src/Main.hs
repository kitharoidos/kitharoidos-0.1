...
----------------------------------------------------------------------------------------------------
main :: IO ()
main = runKitharoidos experiment0

----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
-- Plot envelope and phase
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
--main = do renderableToPNGFile (toRenderable plotEnvelope) 400 300 ("envelope" ++ ".png")
--          renderableToPNGFile (toRenderable plotPhase)    400 300 ("phase"    ++ ".png")
--          return ()
...
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
plotEnvelope :: Layout1 Double Double
plotEnvelope
...
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
plotPhase :: Layout1 Double Double
plotPhase
  = layout1_bottom_axis ^= (laxis_title   ^= "time (s)"    $ defaultLayoutAxis)
  $ layout1_top_axis    ^= (laxis_visible ^= (const False) $ defaultLayoutAxis)
  $ layout1_left_axis   ^= (laxis_title   ^= "phase (rad)" $ defaultLayoutAxis)
  $ layout1_right_axis  ^= (laxis_visible ^= (const False) $ defaultLayoutAxis)
  $ layout1_plots       ^= [ Left . toPlot $
                             (  plot_lines_style  .> line_color ^= (opaque cadetblue)
                              $ plot_lines_values ^= [zip ts (map (flip phi 1) ts)]
                              $ defaultPlotLines
                             )
                           ]
  $ defaultLayout1
...
plotOscAmplitude :: AlphaColour Double -> [(Double, Double)] -> Layout1 Double Double
...
plotOscFreq :: AlphaColour Double -> [(Double, Double)] -> Layout1 Double Double
...
runInputOsc (r1', r2', rmax, ton, toff, freq) ts
  = zip ts . zip inputOsc $ map (const freq) inputOsc
    where inputOsc = zip (map ((-) <$> r r1' rmax ton <*> r r2' rmax toff) ts)
                         (map (flip phi freq) ts)
