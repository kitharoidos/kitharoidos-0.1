      ...
      {getStateT
         = \cont generatorState@(GeneratorState {envs, envIA, buffPtrs, tsim, maxRX}) ->
             let tsim2 = runST $ do
                   ...
                   tsim1     <- newSTRef tsim
                   mapM_ (\ev -> case ev of
                            PMEvent {message = PMMsg {status = 144, data1, data2 = 0}, timestamp} -> do
                            ...
                            PMEvent {message = PMMsg {status = 144, data1, data2}, timestamp} -> do
                              let k    = fromIntegral data1
                                  ix   = envIA V.! k + buffPtrs V.! k
                                  r    = maxRX * fromIntegral data2 / 127
                                  ton  = fromIntegral timestamp / 1000
                              M.unsafeWrite envs1 ix (r, ton, 1 / 0)
                              readSTRef tsim1 >>= \t -> if t < 0 then writeSTRef tsim1 ton else return ()
                            PMEvent {message = PMMsg {status = 176, data1 = 123}} -> do
